...
namespace DigitalOpus.MB.Core{
	...
	public class MB3_EditorMethods : MB2_EditorMethodsInterface{
        ...
        bool _SetTextureFormat2017(Texture2D tx, TextureFormatInfo toThisFormat, bool addToList, bool setNormalMap, TextureImporter textureImporter)
        {
            ...
            if (addToList)
            {
                s = "Setting texture crunch compression for ";
            }
            else
            {
                s = "Restoring texture crunch compression for ";
            }
            s += String.Format("{0}  to compression={1} isNormal={2} ", tx, toThisFormat.doCrunchCompression, setNormalMap);
            if (toThisFormat.platform != null)
            {
                s += String.Format(" setting platform override format for platform {0} to {1} compressionQuality {2}", toThisFormat.platform, toThisFormat.doCrunchCompression, toThisFormat.platformCompressionQuality);
            }
            Debug.Log(s);
            ...
            if (doImport && addToList && !_textureFormatMap.ContainsKey(tx)) _textureFormatMap.Add(tx, restoreTfi);
            return doImport;
        }
